...
class Release(db.Model):
    ...
    status: Mapped[statuses] = mapped_column(db.Enum(statuses), default=statuses.DRAFT)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'status': self.status.value if isinstance(self.status, statuses) else self.status,
